...
#define ENCODER_NAME "luffing"
#define ENCODER_REG_ADDR REG_LUFFING_ENCODER_COUNT
...
void read_luffing_encoder_thread(void *argument){
    ...
    for(;;)
    {
        ...
        if (err != OP_OK_QUERY){
            ...
            /* Set data valid bit for luffing encoder (bit 1) */
            mb_reg_write_bit(REG_DATA_VAILD, 1, 1);
        }
    }
}