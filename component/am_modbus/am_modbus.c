...
void ModbusInit(modbusHandler_t * modH)
{
  ...
  if (numberHandlers < MAX_M_HANDLERS)
  {
	  ...
	  {
		  LOG_E("Failed to create Modbus task, check heap and stack size (free heap: %lu bytes)\n",
...
void ModbusStart(modbusHandler_t * modH)
{
	...
	{
		LOG_E("To use USART_HW_DMA you need to enable it in the am_modbus_config.h file\n");
		return;
	}
...
uint32_t ModbusQuery(modbusHandler_t * modH, modbus_t telegram )
{
	//Add the telegram to the TX tail Queue of Modbus
	if (modH->uModbusType == MB_MASTER)
	{
		telegram.u32CurrentTask = (uint32_t *) osThreadGetId();
		if (xQueueSendToBack(modH->QueueTelegramHandle, &telegram, 0) != pdPASS)
		{
...
void StartTaskModbusMaster(void *argument)
{
  ...
  for(;;)
  {
     ...
     sendErr = SendQuery(modH, telegram);
     if (sendErr != 0)
     {
	 	 modH->i8state = COM_IDLE;
	 	 modH->i8lastError = sendErr;
	 	 modH->u16errCnt++;
