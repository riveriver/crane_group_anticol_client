...
#include "stdbool.h"
static bool enable_dhcp = false;
static ip4_addr_t static_ipaddr;
static ip4_addr_t static_netmask;
static ip4_addr_t static_gw;
void lwip_set_net_param(bool use_dhcp, uint32_t ip, uint32_t netmask, uint32_t gateway){
    enable_dhcp = use_dhcp;
    static_ipaddr.addr = ip;
    static_netmask.addr = netmask;
    static_gw.addr = gateway;
}
/* USER CODE END 2 */
...
/* USER CODE BEGIN 3 */
    ...
    netif_set_addr(&gnetif, &static_ipaddr, &static_netmask, &static_gw);
  }
